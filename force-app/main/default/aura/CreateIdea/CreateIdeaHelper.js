({
    ...
    setStatusFlag : function(component){
        var action = component.get('c.statusFlag');
        action.setCallback(this, function(response){
            var state = response.getState();
            if(state === 'SUCCESS' && component.isValid()){
                component.set("v.statusFlag",response.getReturnValue());
            }
            else if(state === 'ERROR'){
                console.log(reposne.error);
            }else{
    ...
    getAllIdeas : function(component){
        var action = component.get('c.getAllIdeas');
        action.setCallback(this, function(response){
            var state = response.getState();
            if(state === 'SUCCESS' && component.isValid()){
                component.set("v.allIdeas",response.getReturnValue());
            }
            else if(state === 'ERROR'){
                console.log(reposne.error);
            }else{
    ...
    searchDuplicates : function(component,searchKey){
        let allIdeas=component.get("v.allIdeas");
        let duplicateIdeas =[];
        if(searchKey){
            allIdeas.forEach(function(idea){ 
                idea.Title.split(" ").forEach(function(split){
                    if(split.startsWith(searchKey.trim().split(" ")[0]) && idea.Title.includes(searchKey)){
                        console.log(idea);
                        duplicateIdeas.push(idea);
                        return;
                    }
                });
            });
            console.log(duplicateIdeas);
            component.set("v.duplicateIdeas",duplicateIdeas);
            if(duplicateIdeas.length>0){
                component.set("v.findDuplicateFlag",true);
            }
        }
        else{
            component.set("v.findDuplicateFlag",false); 
        }
    },
